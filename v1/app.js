// App State
const AppState = { mode: 'clean', isDark: true };
...
const layoutConfig = [
    { id: 'tab-identity', title: '人物設定', sections: [ { title: '基本資訊', icon: '📝', fields: [ { id: 'gender', label: '性別 (Gender)', source: 'gender', placeholder: '-- 選擇 --' }, { id: 'race', label: '種族與國籍 (Race)', source: 'race' }, { id: 'char-age', label: '年齡 (Age)', source: 'charAge' }, { id: 'char-profession', label: '職業 (Profession)', source: 'charProfession', custom: true } ] }, { title: '體態與身型', icon: '📏', fields: [ { id: 'body-height', label: '身高 (Height)', source: 'bodyHeight' }, { id: 'body-type', label: '體態 (Type)', source: 'bodyType' }, { id: 'body-bust', label: '胸圍 (Bust) - 性感解鎖', source: 'bodyBust', tags: ['sexy', 'nsfw'] }, { id: 'body-waist', label: '腰圍 (Waist)', source: 'bodyWaist' }, { id: 'body-hip', label: '臀圍 (Hip)', source: 'bodyHip' } ] }, { title: '五官與特徵', icon: '✨', fields: [ { id: 'hair-style', label: '髮型 (Hairstyle)', source: 'hairStyle', custom: true }, { id: 'hair-color', label: '髮色 (Hair Color)', source: 'hairColor', custom: true }, { id: 'eye-color', label: '眼睛與瞳色 (Eye Color)', source: 'eyeColor', custom: true }, { id: 'eye-shape', label: '眼型 (Eye Shape)', source: 'eyeShape' }, { id: 'face-shape', label: '臉型 (Face Shape)', source: 'faceShape' }, { id: 'skin-type', label: '膚色與膚質 (Skin)', source: 'skinType', custom: true }, { id: 'body-tattoo', label: '紋身與印記 (Tattoo)', source: 'bodyTattoo', custom: true } ] }, { title: '妝容與臉部', icon: '💄', fields: [ { id: 'makeup-base', label: '底妝 (Base)', source: 'makeupBase' }, { id: 'makeup-eyebrow', label: '眉毛 (Eyebrows)', source: 'makeupEyebrow' }, { id: 'makeup-detail-1', label: '細節妝容 (Detail)', source: 'makeupDetail' } ] } ] },
    { id: 'tab-outfit', title: '服裝穿搭', sections: [ { title: '套裝與物理效果', icon: '👗', fields: [ { id: 'outfit-theme', label: '服裝套裝 (Costume Preset)', source: 'outfitTheme', custom: true }, { id: 'outfit-fit-physics', label: '穿著效果 (Physics)', source: 'outfitFitPhysics', custom: true } ] }, { title: '混搭單品', icon: '👕', fields: [ { id: 'outfit-outer', label: '外套 (Outerwear)', source: 'outfitOuter' }, { id: 'outfit-top', label: '上衣 (Top)', source: 'outfitTop', custom: true }, { id: 'outfit-bottom', label: '下著 (Bottom)', source: 'outfitBottom', custom: true }, { id: 'outfit-sock', label: '襪子 (Socks)', source: 'outfitSock' }, { id: 'outfit-shoe', label: '鞋子 (Shoes)', source: 'outfitShoe' } ] }, { title: '飾品配件', icon: '💍', fields: [ { id: 'outfit-head', label: '頭飾與面具 (Headwear)', source: 'outfitHead' }, { id: 'outfit-acc', label: '飾品 (Jewelry)', source: 'outfitAcc' }, { id: 'outfit-hands', label: '手部配件 (Hands)', source: 'outfitHands' }, { id: 'outfit-bag', label: '包包 (Bag)', source: 'outfitBag' } ] } ] },
    { id: 'tab-action', title: '動作與表情', sections: [ { title: '基礎動作', icon: '🏃', fields: [ { id: 'action-pose', label: '姿勢 (Pose)', source: 'actionPose', custom: true }, { id: 'action-activity', label: '互動行為 (Activity)', source: 'actionActivity', custom: true }, { id: 'action-expression', label: '表情 (Expression)', source: 'actionExpression', custom: true } ] } ] },
    { id: 'tab-scene', title: '場景與特效', sections: [ { title: '環境設定', icon: '🏞️', fields: [ { id: 'scene-loc', label: '場景 (Location)', source: 'sceneLoc', custom: true }, { id: 'scene-time', label: '時間 (Time)', source: 'sceneTime' }, { id: 'scene-weather', label: '天氣 (Weather)', source: 'sceneWeather' }, { id: 'scene-lighting', label: '光影 (Lighting)', source: 'sceneLighting' } ] }, { title: '物件庫', icon: '🛋️', fields: [ { id: 'scene-props', label: '環境裝飾 (Props)', source: 'sceneProps', custom: true }, { id: 'scene-props-detail', label: '裝飾細節 (Detail)', source: 'scenePropsDetail' } ] } ] },
    { id: 'tab-camera', title: '鏡頭與風格', sections: [ { title: '攝影參數', icon: '📷', fields: [ { id: 'cam-lens', label: '焦段 (Lens)', source: 'camLens' }, { id: 'cam-aperture', label: '光圈 (Aperture)', source: 'camAperture' }, { id: 'cam-shot', label: '景別 (Shot)', source: 'camShot' }, { id: 'cam-angle', label: '視角 (Angle)', source: 'camAngle' }, { id: 'scene-composition', label: '構圖 (Composition)', source: 'sceneComposition' } ] }, { title: '風格疊加', icon: '🎨', fields: [ { id: 'scene-style-realistic', label: '基礎風格 (Style)', source: 'sceneStyleRealistic', custom: true }, { id: 'scene-style-special', label: '特殊藝術濾鏡 (Special)', source: 'sceneStyleSpecial' } ] } ] }
];
...
function buildLayout() {
    ...
    layoutConfig.forEach((tabObj, index) => {
        ...
        tabObj.sections.forEach(sec => {
            ...
            sec.fields.forEach(f => {
                ...
                if (f.custom) selectHtml += `<input type="text" id="${f.id}-custom" class="input-control custom-input" placeholder="自訂...">`;
    ...
    populateSelects();
    document.querySelector('.editor-section').addEventListener('change', () => { UI.updateVisuals(); PromptEngine.build(); });
    document.querySelector('.editor-section').addEventListener('keyup', (e) => { if(e.target.tagName === 'INPUT') { UI.updateVisuals(); PromptEngine.build(); }});
}
    ...
    document.querySelectorAll('select.input-control').forEach(select => {
        ...
        if (fieldTagsAttr) {
            ...
            if (fieldTags.length > 0 && !fieldTags.includes(AppState.mode)) {
                select.parentElement.style.display = 'none'; select.value = ""; return;
            } else select.parentElement.style.display = 'flex';
        }
...
const PromptEngine = {
    ...
    build: () => {
        ...
        let features = [
            ...
            PromptEngine.v('skin-type'), PromptEngine.v('body-height'), PromptEngine.v('body-type'), PromptEngine.v('body-bust'),PromptEngine.v('body-waist'), PromptEngine.v('body-hip'), PromptEngine.v('body-tattoo')
        ...
        let outfitsZh = [
            ...
            PromptEngine.getText('outfit-outer'), [PromptEngine.getText('outfit-top'), PromptEngine.v('outfit-top-custom')].filter(Boolean).join(""), [PromptEngine.getText('outfit-bottom'), PromptEngine.v('outfit-bottom-custom')].filter(Boolean).join(""), PromptEngine.getText('outfit-sock'), PromptEngine.getText('outfit-shoe')
        ].filter(Boolean);
    ...
    randomize: () => {
        document.querySelectorAll('select.input-control').forEach(s => {
            if (s.options.length > 2 && s.parentElement.style.display !== 'none' && Math.random() > 0.6) s.selectedIndex = Math.floor(Math.random() * (s.options.length - 1)) + 1;
...
window.onload = () => { buildLayout(); setMode('clean'); };
